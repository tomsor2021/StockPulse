...
> [!IMPORTANT]
> **部署模式选择建议：**
> - **个人开发**：使用本地模式（`npx`），功能最全
> - **团队协作 / 远程服务**：使用腾讯云托管模式（方式二），免运维
> - **自建服务器**：务必设置 `CLOUDBASE_MCP_CLOUD_MODE=true`，禁用本地文件操作类工具
...
<details>
<summary><b>在服务器上部署 MCP 服务是否安全？</b></summary>

CloudBase MCP 提供了完善的部署模式安全隔离机制：

- **本地模式**（`npx` 启动）：MCP 服务运行在开发者本机，通过 stdio 与 AI IDE 通信。调用方就是开发者本人，拥有该机器的完整控制权。`downloadRemoteFile`、`manageCloudRun(action="run")` 等工具本质上等同于开发者在终端执行 `curl` 或 `node app.js`，这是产品的正常功能。
- **云端/服务器模式**（设置 `CLOUDBASE_MCP_CLOUD_MODE=true`）：启用三层安全过滤机制，**所有涉及本地文件读写、本地进程启动的工具均被禁用**，远程调用方无法通过 MCP 操作服务器本地文件系统或启动本地进程。

如果你需要在远程服务器上自行部署 MCP 服务，**务必设置 `CLOUDBASE_MCP_CLOUD_MODE=true` 环境变量**。使用腾讯云托管模式（Streamable HTTP）时已自动启用该保护。
